...
namespace FirstPerson.Player
{
    public class PlayerExamine : MonoBehaviour
    {
        ...
        [SerializeField] private LayerMask _layersToReach;
        ...
        public void Examine()
        {
            if (Physics.Raycast(_cameraTransform.position, _cameraTransform.forward, out RaycastHit hit, _distanceToExamine, _visibleLayers) &&
                hit.transform && _layersToExamine == (_layersToExamine | (1 << hit.transform.gameObject.layer)))
            {
        ...
        {
            if (_layersToReach == (_layersToReach | (1 << hit.transform.gameObject.layer)))
            {
